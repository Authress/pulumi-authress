PROJECT_NAME := authress Package

SHELL            := /bin/bash
PACK             := authress
ORG              := Authress
PROJECT          := github.com/${ORG}/pulumi-${PACK}
NODE_MODULE_NAME := @authress/${PACK}
TF_NAME          := ${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-authress,${REPOSITORY},g' provider/go.mod; \
...
install_provider::
	pulumi plugin install resource pulumi-resource-authress ${VERSION} --file ./provider/cmd/pulumi-resource-authress
